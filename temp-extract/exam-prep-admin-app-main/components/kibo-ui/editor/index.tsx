...
export const defaultSlashSuggestions: SuggestionOptions<SuggestionItem>["items"] =
  () => [
    ...
    },
    {
      ...
      searchTerms: ["math", "latex", "equation", "formula", "katex"],
      icon: SigmaIcon,
      command: ({ editor, range }) => {
        editor.chain().focus().deleteRange(range).run();
        const latex = window.prompt("Enter LaTeX equation (e.g., E = mc^2):");
        if (latex) {
    ...
    {
      title: "Video",
      description: "Embed a video (YouTube, Vimeo, or direct URL).",
    ...
    },
    {
      ...
      searchTerms: ["audio", "sound", "music", "mp3", "podcast"],
      icon: AudioLinesIcon,
      command: ({ editor, range }) => {
        editor.chain().focus().deleteRange(range).run();
...
export const EditorLinkSelector = ({
  ...
  // Set initial URL from existing link when popover opens
  useEffect(() => {
    if (open && editor) {
      const existingHref = (editor.getAttributes("link") as { href?: string }).href;
      setUrl(existingHref ?? "");
