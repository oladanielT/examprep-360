...
export const useTutorials = (params: TutorialListParams = {}) => {
  ...
  return useQuery<TutorialListItem[]>({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get<TutorialListResponse>(
        TUTORIALS_ENDPOINTS.LIST,
        {
          params,
        }
      );
...
export const useTutorial = (id: string) => {
  ...
  return useQuery<TutorialDetail>({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get<TutorialDetail>(
        TUTORIALS_ENDPOINTS.DETAILS(id)
      );
      return data;
    },
...
export const useBookmarkedTutorials = () => {
  ...
  return useQuery<BookmarkedTutorial[]>({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get<BookmarkedTutorial[]>(
        TUTORIALS_ENDPOINTS.BOOKMARKS
      );
      return data;
    },
    enabled: isAuthenticated,
